      ...
      <ScrollView ref={listRef} style={{ flex: 1 }}
        ...
        <View style={s.header}>
          <View style={s.headerCopy}>
            ...
            <Text numberOfLines={1} style={[type.tiny, s.muted]}>살다 보면, 누구 잘못인지 궁금한 순간.</Text>
          </View>
        ...
        <Pressable accessibilityRole="button" onPress={() => router.push('/onboarding')}
          ...
          <Text numberOfLines={1} style={[type.tiny, s.muted, s.fixed]}>이용 안내</Text>
          <Text numberOfLines={1} style={[type.small, s.ink, s.flexText]}>처음 오셨나요? 이렇게 참여해요</Text>
        ...
        {filter === 'today' && (
          <View style={s.progressRow}>
            <Text numberOfLines={1} style={[type.tiny, s.muted, s.flexText]}>{done}/{todayCases.length}건 참여 · 매일 09:00 갱신</Text>
            {next && <Pressable accessibilityRole="button" onPress={() => router.push(`/case/${next.id}`)} style={s.textButton}>
              <Text numberOfLines={1} style={[type.tiny, s.ink]}>{done ? '이어서' : '첫 판결'} ›</Text>
            </Pressable>}
          </View>
        )}
