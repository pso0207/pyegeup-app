...
현재 이 Mac에는 MySQL 8.0.46이 설치되어 있으므로 실제 실습은 [실습 프로젝트](learning/PRACTICE-PROJECT.md)에서 로컬 MySQL을 사용합니다. 아래 Docker MySQL 8.4 장은 환경을 격리하고 싶을 때 선택하는 대안이며 둘을 동시에 실행하지 않습니다.
