...
- **선고 순간만 암전** — 개표 3초 동안 화면이 검게 내려앉았다가 확정과 함께 조명이 올라온다(`app/case/result.js` + `theme.dark`). 처음부터 어두우면 이 연출이 배경에 묻힌다.
- **목록은 카드가 아니라 행** — `CaseList` + `CaseCard`. 제목 1줄 · 메타 1줄 고정이라 행 높이가 전부 같고, 한 화면에 사건이 2.5개가 아니라 6~7개 뜬다.
